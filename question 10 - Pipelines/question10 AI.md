...
```bash
oc create serviceaccount pipeline -n cicd
...
### 4. Create the trigger RBAC after the student prepares it

The student must not create Roles or RoleBindings. Those resources grant permissions and require an administrator. After the student prepares `trigger-rbac.yaml`, the administrator must create the Role and RoleBindings in `cicd`:

```bash
oc apply -f trigger-rbac.yaml -n cicd
```

The administrator must verify the active user is `admin` or `kubeadmin` before applying this file. The developer must not run this command.
...
Create `triggers.yaml` as the `developer` user. It must contain the EventListener ServiceAccount, TriggerBinding, TriggerTemplate, and EventListener. Do not apply it until the administrator has created the Role and RoleBindings from `trigger-rbac.yaml`.

Prepare `trigger-rbac.yaml`, containing the Role and RoleBindings that allow the EventListener ServiceAccount to create PipelineRuns, plus the required ClusterRoleBinding for Tekton ClusterInterceptors. Give that file to the administrator to apply before applying `triggers.yaml`.

After the administrator applies the RBAC file, wait until the EventListener pod is ready before exposing its Service or sending a trigger request.
